...
def get_new_path_for_file(src_file):
    ...
    sub_path_suffix = "" if file_type == "img" else f"_{file_type}"
    new_path = file_timestamp.strftime("%Y-%m") + sub_path_suffix
...
def check_files(dest_dir, src_dir):
    ...
    if src_hashes == dest_hashes:
        print(f"{len(dest_hashes)}/{len(src_hashes)} reorganised successfully")        
        return True
